...
if args_namespace.help or (not args_namespace.update \
    ...
    logo()
    print("Scan is updating....Please wait.\n")
    spinner.start()
    ...
    while(tool < len(tool_names)):
        ...
        temp_file = "/tmp/scan_temp_"+tool_names[tool][arg1]
    ...
    if len(rs_vul_list)==0:
        ...
        with open("RS-Vulnerability-Report", "a") as report:
            while(rs_vul < len(rs_vul_list)):
                ...
                report.write("\n------------------------\n\n")
                temp_report_name = "/tmp/scan_temp_"+vuln_info[arg1]
    ...
    for file_index, file_name in enumerate(tool_names):
        with open("RS-Debug-ScanLog", "a") as report:
            try:
                with open("/tmp/scan_temp_"+file_name[arg1], 'r') as temp_report:
                        data = temp_report.read()
    ...
    os.system('rm /tmp/scan_te* > /dev/null 2>&1') # Clearing previous scan files
